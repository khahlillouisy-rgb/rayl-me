...
function renderSocialIcons() {
  ...
  const icons = platforms
    ...
    .map(p => `<a href="${siteSettings[`social_${p}`]}" target="_blank" rel="noopener noreferrer" class="social-icon" title="${p}">${SOCIAL_SVGS[p]}</a>`)
    .join('');
...
function staticHero() {
  return `<div class="hero">
    ...
    <div class="hero-side">
      <div class="hero-side-item"><div><span class="tag tag-bodies">Bodies</span><div class="hero-side-title">The STI conversation nobody is having at college orientation</div></div><div class="hero-side-byline">Anonymous &nbsp;·&nbsp; 6 min read</div></div>
      <div class="hero-side-item"><div><span class="tag tag-verified">Verified</span><div class="hero-side-title">"Health transparency shouldn't feel like a clinical transaction"</div></div><div class="hero-side-byline">Interview &nbsp;·&nbsp; 8 min read</div></div>
    </div>
  </div>`;
}
