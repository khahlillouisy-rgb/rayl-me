...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Enable styleWithCSS globally so all execCommands produce inline CSS
...
// KEY: always use onmousedown + preventDefault to keep editor focus
// KEY: always call styleWithCSS before formatting commands
...
// Wrap selected text in a styled element — works even when execCommand fails
function wrapSelection(tag, styles) {
  ...
  const styleStr = Object.entries(styles)
    ...
    .join(';');
  // Use insertHTML which is more reliable cross-browser
  const selectedText = range.toString();
  if (!selectedText) return;
  document.execCommand('insertHTML', false, `<span style="${styleStr}">${selectedText}</span>`);
}
