// admin.js — Rayl Me admin with rich text editor + Supabase
...
function goSection(id) {
  ...
  if (id==='subscribers') renderSubscribers();
}

// DASHBOARD
async function renderDashboard() {
  try {
    const [posts,subs,users] = await Promise.all([dbGetAllPosts(),dbGetSubscribers(),dbGetUsers()]);
    ...
    document.getElementById('dash-recent').innerHTML = posts.slice(0,5).map(p=>
      `<tr><td class="td-title">${x(p.title)}</td><td><span class="tag tag-${p.category}">${cl(p.category)}</span></td><td><span class="td-status-${p.status}">${p.status}</span></td><td>${fmtDate(p.created_at)}</td></tr>`
...
async function renderPosts() {
  ...
  try {
    ...
    document.getElementById('posts-table').innerHTML = allPosts.length
      ? allPosts.map(p=>`<tr>
          <td class="td-title">${x(p.title)}</td>
          <td><span class="tag tag-${p.category}">${cl(p.category)}</span></td>
          <td style="color:#555;">${[p.image_url&&'🖼',p.video_url&&'▶',p.audio_url&&'♪'].filter(Boolean).join(' ')||'—'}</td>
...
async function savePost(forcedStatus) {
  ...
  try {
    ...
    const post = {
      title, category:document.getElementById('p-cat').value,
      status:forcedStatus,
    ...
    if (editingPostId) { await dbUpdatePost(editingPostId,post); toast('Post updated ✓'); }
    else { await dbCreatePost(post); toast(forcedStatus==='published'?'Published ✓':'Draft saved ✓'); }
...
function cmdBlock(tag) {
  if (!tag) return;
  document.getElementById('editor').focus();
  document.execCommand('formatBlock', false, tag);
}

function cmdFont(font) {
  if (!font) return;
...
function insertDivider() {
  restoreSelection();
  document.execCommand('insertHTML', false, '<hr style="border:none;border-top:0.5px solid #ddd;margin:28px 0;"><p><br></p>');
}
...
function handleImageUpload(input) {
  ...
  showImageCurrent(URL.createObjectURL(file), file.name);
  input.value='';
}
function showImageCurrent(url, name) {
  document.getElementById('img-upload-area').style.display='none';
  const el=document.getElementById('img-current');
...
function showAudioCurrent(nameOrUrl) {
  document.getElementById('audio-upload-area').style.display='none';
  const el=document.getElementById('audio-current');
  el.style.display='flex';
  el.innerHTML=`<span style="font-size:18px;">♪</span><span class="current-media-name">${nameOrUrl.split('/').pop()}</span><span class="current-media-remove" onclick="clearAudio()">✕</span>`;
}
...
async function renderCategories() {
  try {
    ...
    document.getElementById('cats-table').innerHTML=CATEGORIES.map(c=>{
      ...
      return`<tr><td><span class="tag tag-${c.id}">${c.label}</span></td><td style="color:#555;font-family:var(--mono);font-size:11px;">${c.id}</td><td style="color:#888;font-family:var(--mono);font-size:11px;">${n}</td></tr>`;
    }).join('');
  } catch(e) { console.error(e); }
}
...
async function renderUsers() {
  ...
  try {
    ...
    const adminRow=`<tr><td class="td-title">Khahlil Louisy</td><td style="color:#888;font-family:var(--mono);font-size:11px;">${ADMIN_EMAIL}</td><td><span class="role-badge role-publisher">Publisher</span></td><td style="color:#555;font-family:var(--mono);font-size:11px;">Owner</td><td></td></tr>`;
    document.getElementById('users-table').innerHTML=adminRow+allUsers.map(u=>`<tr>
      <td class="td-title">${x(u.fname+' '+(u.lname||''))}</td>
      <td style="color:#888;font-family:var(--mono);font-size:11px;">${x(u.email)}</td>
      <td><span class="role-badge role-${u.role}">${ROLE_LABELS[u.role]||u.role}</span></td>
      <td style="color:#555;font-family:var(--mono);font-size:11px;">${fmtDate(u.created_at)}</td>
      <td class="td-actions">
...
async function renderSubscribers() {
  ...
  try {
    ...
    document.getElementById('subs-table').innerHTML=allSubs.length
      ?allSubs.map(s=>`<tr><td style="color:#e8e4dc;font-family:var(--mono);font-size:11px;">${x(s.email)}</td><td style="color:#555;font-family:var(--mono);font-size:11px;">${s.source||'homepage'}</td><td style="color:#555;font-family:var(--mono);font-size:11px;">${fmtDate(s.created_at)}</td></tr>`).join('')
...
function loadingRow(c){return`<tr><td colspan="${c}" style="color:#333;text-align:center;padding:24px;font-family:var(--mono);font-size:11px;">Loading…</td></tr>`;}
function emptyRow(c,m){return`<tr><td colspan="${c}" style="color:#333;text-align:center;padding:28px;font-family:var(--mono);font-size:11px;">${m}</td></tr>`;}
